...
final class AudioProcessorTests: XCTestCase {
    ...
    func testSimulatedAudioProcessing() async throws {
        ...
        for sampleIndex in 0..<sampleCount {
            let phase = 2.0 * Float.pi * Float(frequency) * Float(sampleIndex) / Float(sampleRate)
            sineWave[sampleIndex] = amplitude * sin(phase)
        }
        ...
        for index in 0..<fftData.count where fftData[index] > maxValue {
            maxValue = fftData[index]
            maxIndex = index
        }

        // Verify the peak is in the expected range        // Verify the peak is in the expected range
