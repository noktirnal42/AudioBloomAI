...
final class MetalComputeCoreTests: XCTestCase {
    ...
    func testBufferUpdateAndRead() throws {
        ...
        for index in 0..<bufferSize {
            XCTAssertEqual(readData[i], 42.0, "Data should match at index \(i)")
        }
    }
    ...
    func testErrorHandling() throws {
        ...
        XCTAssertThrowsError(try metalCore.readBuffer(
            id: bufferId,
            into: UnsafeMutableRawPointer.allocate(byteCount: (bufferSize + 10) * MemoryLayout<Float>.stride, alignment: 4),
    ...
    func testFFTProcessing() async throws {
        ...
        let binError = 2
        XCTAssertTrue(abs(maxBin - expectedBin) <= binError, "FFT should detect frequency at correct bin (expected: \(expectedBin), got: \(maxBin))")
    }
    ...
    func testFrequencyDomainFiltering() async throws {
        ...
        for index in 0..<cutoffBin {
            // Set filter coefficients to 1.0 (pass)
            filter[i * 2] = 1.0     // Real part
            filter[i * 2 + 1] = 0.0 // Imaginary part
        }
        ...
        for index in 0..<(fftSize / 2) {
            let realOut = outputData[i * 2]
            let imagOut = outputData[i * 2 + 1]

            if i < cutoffBin {
                // For bins below cutoff, output should match input
                XCTAssertEqual(realOut, Float(i * 2), accuracy: 0.001, "Low frequency real component should be preserved")
                XCTAssertEqual(imagOut, Float(i * 2 + 1), accuracy: 0.001, "Low frequency imaginary component should be preserved")
            } else {
                // For bins above cutoff, output should be attenuated
                XCTAssertEqual(realOut, 0.0, accuracy: 0.001, "High frequency real component should be attenuated")
                XCTAssertEqual(imagOut, 0.0, accuracy: 0.001, "High frequency imaginary component should be attenuated")
            }
        }
    }
    ...
    func testSpectrumAnalysis() async throws {
        ...
        for index in 0..<fftSize {
            let bassSample = sin(2.0 * Float.pi * bassFreq * Float(i) / sampleRate)
            let midSample = sin(2.0 * Float.pi * midFreq * Float(i) / sampleRate)
            let trebleSample = sin(2.0 * Float.pi * trebleFreq * Float(i) / sampleRate)

            // Combine signals with different amplitudes
            signal[i] = bassSample * 0.7 + midSample * 0.5 + trebleSample * 0.3
        }
    ...
    func testAudioNormalization() async throws {
        ...
        for index in 0..<sampleCount {
            let phase = 2.0 * Float.pi * Float(i) / 100.0
            testAudio[i] = peakValue * sin(phase)
        }
        ...
        for index in 0..<min(10, sampleCount) {
            let expectedSample = testAudio[i] * expectedScaleFactor
            XCTAssertEqual(normalizedAudio[i], expectedSample, accuracy: 0.001, "Sample at index \(i) should be scaled correctly")
        }
    }
    ...
    func testConcurrentProcessing() async throws {
        ...
        let stats = metalCore.getBufferStats()
        print("After concurrent processing: active buffers = \(stats.activeCount), pooled buffers = \(stats.pooledCount)")
    }
